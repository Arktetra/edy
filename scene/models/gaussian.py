...
from utils.graphics import scaling_rotation, quaternion_to_rotation
from utils import dist_square_nearest_k
...
class GaussianModel:
    ...
    def restore(self, model_args, training_args, device):
        (
        ...
        self.training_setup(training_args, device)
    ...
    def one_up_SH_degreee(self):
        if self.active_sh_degree < self.max_sh_degree:
            self.active_sh_degree += 1

    def create_from_pcd(self, pcd: BasicPointCloud, cam_infos, spatial_lr_scale: float, device: str):
        self.spatial_lr_scale = spatial_lr_scale

        fused_point_cloud = torch.tensor(np.asarray(pcd.points)).float().to(device)
        fused_color = rgb_to_sh(torch.tensor(np.asarray(pcd.colors)).float().to(device))
        ...
        mean_dists_2 = torch.clamp_min(
            dist_square_nearest_k(torch.from_numpy(np.asarray(pcd.points)).float().to(device)), 
            0.0000001
        )
        scales = torch.log(torch.sqrt(mean_dists_2))[..., None].repeat(1, 3)
    ...
    def training_setup(self, training_args, device):
        ...
        self.denom = torch.zeros((self.get_xyz[0], 1), device=device)
        ...
        self.xyz_scheduler_args = get_expon_lr_func(
            ...
            lr_final=training_args.position_lr_init * self.spatial_lr_scale,
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2, device="cuda"):
        ...
        rots = quaternion_to_rotation(self._rotation[selected_pts_mask]).repeat(N, 1, 1)
        ...
        self.densification_postfix(
            new_xyz, new_features_dc, new_features_rest, new_opacity, new_scaling, new_rotation, new_tmp_radii, device
        )
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent, device):
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_tmp_radii, device)

    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size, radii, device):
