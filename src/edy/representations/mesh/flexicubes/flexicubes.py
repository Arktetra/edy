...
# from kaolin.utils.testing import check_tensor # prev. taken from
# replacing with utils that has the definition of this
...
class FlexiCubes:
    ...
    def __call__(
        ...
        assert torch.is_tensor(voxelgrid_vertices) and check_tensor(voxelgrid_vertices, (None, 3), throw=False), (
            "'voxelgrid_vertices' should be a tensor of shape (num_vertices, 3)"
        )
        num_vertices = voxelgrid_vertices.shape[0]
        assert torch.is_tensor(scalar_field) and check_tensor(scalar_field, (num_vertices,), throw=False), (
        ...
        assert beta is None or (torch.is_tensor(beta) and check_tensor(beta, (num_cubes, 12), throw=False)), (
            "'beta' should be a tensor of shape (num_cubes, 12)"
        )
        assert alpha is None or (torch.is_tensor(alpha) and check_tensor(alpha, (num_cubes, 8), throw=False)), (
            "'alpha' should be a tensor of shape (num_cubes, 8)"
        )
        assert gamma_f is None or (torch.is_tensor(gamma_f) and check_tensor(gamma_f, (num_cubes,), throw=False)), (
    ...
    def _compute_vd(
        ...
        surf_edges_x = torch.index_select(input=voxelgrid_vertices, index=surf_edges.reshape(-1), dim=0).reshape(
            -1, 2, 3
        )
        ...
        if voxelgrid_colors is not None:
            ...
            surf_edges_c = torch.index_select(input=voxelgrid_colors, index=surf_edges.reshape(-1), dim=0).reshape(
                -1, 2, C
            )
        ...
        for num in torch.unique(num_vd):
            ...
            curr_edge_group_to_vd = (
                torch.arange(curr_num_vd, device=self.device).unsqueeze(-1).repeat(1, 7) + total_num_vd
            )
        ...
        beta_group = torch.gather(input=beta.reshape(-1), dim=0, index=edge_group_to_cube * 12 + edge_group).reshape(
            -1, 1
        )
    ...
    def _triangulate(
        ...
        if not training:
            ...
            if vd_color is not None:
                color_quad = torch.index_select(input=vd_color, index=quad_vd_idx.reshape(-1), dim=0).reshape(
                    -1, 4, vd_color.shape[-1]
                )
