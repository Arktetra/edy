...
def render_mesh_paper(mesh, mv, mvp, iter_res, return_types=["mask", "depth"], white_bg=False):
    ...
    for type in return_types:
        if type == "mask":
            ...
            normal_indices = (torch.arange(0, mesh.nrm.shape[0], dtype=torch.int64, device="cuda")[:, None]).repeat(
                1, 3
            )
...
class SplitVisualizer:
    ...
    def render(self, camera):
        ...
        outputs = {
            k: torch.cat([lh_outputs[k][0].permute(1, 0, 2), rh_outputs[k][0].permute(1, 0, 2)], dim=0).permute(1, 0, 2)
            for k in ["normals", "wireframe"]
        }
        return {
            "img": (outputs["wireframe"] * ((outputs["normals"] + 1.0) / 2.0) * 255).to(torch.uint8),
            "normals": outputs["normals"],
        }

    def show(self, init_camera):
        visualizer = kal.visualize.IpyTurntableVisualizer(
            self.height, self.width * 2, copy.deepcopy(init_camera), self.render, max_fps=24, world_up_axis=1
        )
...
class TimelineVisualizer:
    ...
    def render(self, camera):
        ...
        return {
            "img": (outputs["wireframe"] * ((outputs["normals"] + 1.0) / 2.0) * 255).to(torch.uint8)[0],
            "normals": outputs["normals"][0],
        }

    def show(self, init_camera):
        visualizer = kal.visualize.IpyTurntableVisualizer(
            self.height, self.width, copy.deepcopy(init_camera), self.render, max_fps=24, world_up_axis=1
        )
        ...
        idx_slider = IntSlider(
            value=self.idx, min=0, max=len(self.meshes) - 1, description="idx", continuous_update=True, readout=True
        )

        interactive_slider = interactive(
            slider_callback, new_wireframe_thickness=wireframe_thickness_slider, new_idx=idx_slider
        )
