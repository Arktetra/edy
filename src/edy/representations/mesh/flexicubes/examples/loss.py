...
def mesh_developable_reg(mesh):
    ...
    for iC in range(3):  # loop over three corners of each triangle
        ...
        vert_degree = torch_scatter.scatter_add(
            src=torch.ones(F, dtype=torch.int32, device=device), index=tris[:, iC], dim=-1, out=vert_degree
        )

    # The energy is the smallest eigenvalue of the outer-product matrix
    vert_normal_covariance_sum = vert_normal_covariance_sum.reshape(-1, 3, 3)  # reshape to a batch of matrices
    ...
    vert_area = torch.where(
        vert_degree == 3, torch.tensor(0, dtype=vert_area.dtype, device=vert_area.device), vert_area
    )
