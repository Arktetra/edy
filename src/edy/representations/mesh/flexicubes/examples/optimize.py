...
if __name__ == "__main__":
    ...
    for it in range(FLAGS.iter):
        ...
        depth_loss = (
            (((buffers["depth"] - (target["depth"])) * target["mask"]) ** 2).sum(-1) + 1e-8
        ).sqrt().mean() * 10

        t_iter = it / FLAGS.iter
        sdf_weight = FLAGS.sdf_regularizer - (FLAGS.sdf_regularizer - FLAGS.sdf_regularizer / 20) * min(
            1.0, 4.0 * t_iter
        )
        reg_loss = (
        ...
        if it % FLAGS.save_interval == 0 or it == (FLAGS.iter - 1):  # save normal image for visualization
            with torch.no_grad():
                ...
                mv, mvp = render.get_rotate_camera(
                    it // FLAGS.save_interval, iter_res=FLAGS.display_res, device=device, use_kaolin=False
                )
                ...
                imageio.imwrite(
                    os.path.join(FLAGS.out_dir, "{:04d}.png".format(it)), np.concatenate([val_image, gt_image], 1)
                )
    ...
    mesh_np = trimesh.Trimesh(
        vertices=vertices.detach().cpu().numpy(), faces=faces.detach().cpu().numpy(), process=False
    )
