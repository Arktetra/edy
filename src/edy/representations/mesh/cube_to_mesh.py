...
from edy.representations.mesh.flexicubes import FlexiCubes
from edy.modules.sparse.tensor import SparseTensor
from edy.representations.mesh.utils import *
...
class SparseFeaturesToMesh:
    """
    Extracts a mesh from sparse features structures using flexicube.

    Args:
        device (str): the device on which the extractor will be run. Defaults to `"cuda"`.
        res (int): the resolution of the dense grid. Defaults to `64`.
        use_color (bool): decides whether to use the color channels or not. Defaults to `True`.
    """

    def __init__(self, device: str = "cuda", res: int = 64, use_color: bool = True):
        super().__init__()
        self.device = device
        self.res = res
