...
def construct_dense_grid(res: int, device: str = "cuda"):
    res_v = res + 1  # resolution of vertices
    verts_id = torch.arange(res_v**3, device=device)
    coords_id = verts_id.reshape(res_v, res_v, res_v)[:res, :res, :res].flatten()
    cube_corners_bias = (cube_corners[:, 0] * res_v + cube_corners[:, 1]) * res_v + cube_corners[:, 2]
    cube_fx8 = coords_id.unsqueeze(1) + cube_corners_bias.unsqueeze(0).to(device)
    verts = torch.stack([verts_id // (res_v**2), (verts_id // res_v) % res_v, verts_id % res_v], dim=1)
